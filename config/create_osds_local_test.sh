...
if ! temp=$(getopt -o d:fuh --long device:,force,unformat,help -n 'note' -- "$@" 2>&1)
...
function create_osd()
{
		...
		then
			...
			return 1
                ...
                if ! ret_err=$(start ceph-osd id=$osd_id 2>&1)
		then
			LAST_ERROR_INFO="$ret_err"
			add_log "ERROR" "$ret_err" $print_log
			return 1
...
function rollback_osds()
{
	...
	for((i=0; i<$osd_total_count; ++i))
		...
		remove_one_osd "${arr_all_osd_id[$i]}" 0
                ...
                local osd_data="${osd_data_dir}/${osd_data_base}-${arr_all_osd_id[$i]}"
		if [ -d "$osd_data" ]
		then
