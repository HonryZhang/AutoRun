...
def main(args):
    ...
    for nodeObj in nodeList:
        ...
        while ( firOsdId == secOsdId):
            secOsdId = random.randint(0, len(osdObjList)-1)
        osdObjList[firOsdId].shutdown(caseName, nodeObj)
        osdObjList[secOsdId].shutdown(caseName, nodeObj)
        for client in clusterObj.getClients():
            client.checkIOError(caseName)
        #start osd service
        ...
        if(status == 'HEALTH_OK'):
            logging.getLogger(caseName).info("stop two osds in cluster successfully")
        else:
